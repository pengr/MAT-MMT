...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers,
                            image_suffix=None, append_eos=True, already_numberized=False):
        if image_suffix:
    ...
    # <<fix>>
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1, image_suffix=None):
        if args.dataset_impl == "raw":
            ...
            make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers, image_suffix=image_suffix)

    # <<fix>>
    def make_all(lang, vocab, image_suffix=None):
        if args.trainpref:
            make_dataset(vocab, args.trainpref, "train", lang, num_workers=args.workers, image_suffix=image_suffix)
        if args.validpref:
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(k) if k > 0 else "valid"
                make_dataset(vocab, validpref, outprefix, lang, num_workers=args.workers, image_suffix=image_suffix)
        if args.testpref:
            for k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "test{}".format(k) if k > 0 else "test"
                make_dataset(vocab, testpref, outprefix, lang, num_workers=args.workers, image_suffix=image_suffix)
