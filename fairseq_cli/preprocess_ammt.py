...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers,
                            label_suffix=None, image_suffix=None, append_eos=True, already_numberized=False):
        if label_suffix:  # <fix>
            print("preprocesing label info......")
            logger.info("[{}] Dictionary: {} types".format(label_suffix, len(vocab) - 1))
            input_prefix += f'.{label_suffix}'
            output_prefix += f'.{label_suffix}'
            append_eos = False
        elif image_suffix:
            print("preprocesing image info......")
            logger.info("[{}] Dictionary: {} types".format(image_suffix, len(vocab) - 1))
            input_prefix += f'.{image_suffix}'
            output_prefix += f'.{image_suffix}'
            append_eos = False
        ...
        merge_result(
            Binarizer.binarize(
                ...
                offset=0, end=offsets[1], append_eos=append_eos, already_numberized=already_numberized  # <fix>
            )
        )
        if num_workers > 1:
            pool.join()
    ...
    # <<fix>>
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1, label_suffix=None, image_suffix=None):
        if args.dataset_impl == "raw":
            ...
            make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers, label_suffix=label_suffix, image_suffix=image_suffix)

    # <<fix>>
    def make_all(lang, vocab, label_suffix=None, image_suffix=None):
        if args.trainpref:
            make_dataset(vocab, args.trainpref, "train", lang, num_workers=args.workers, label_suffix=label_suffix, image_suffix=image_suffix)
        if args.validpref:
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(k) if k > 0 else "valid"
                make_dataset(vocab, validpref, outprefix, lang, num_workers=args.workers, label_suffix=label_suffix, image_suffix=image_suffix)
        if args.testpref:
            for k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "test{}".format(k) if k > 0 else "test"
                make_dataset(vocab, testpref, outprefix, lang, num_workers=args.workers, label_suffix=label_suffix, image_suffix=image_suffix)
    ...
    make_all(None, src_dict, label_suffix=args.label_suffix) # <<fix>>
    make_all(None, src_dict, image_suffix=args.image_suffix)   # <<fix>>
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, append_eos=True, already_numberized=False):
    ...
    res = Binarizer.binarize(filename, vocab, consumer, append_eos=append_eos,
                             offset=offset, end=end, already_numberized=already_numberized) # <fix>
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
