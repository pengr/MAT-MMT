...
def main(args):
    ...
    # <<fix>>
    def make_binary_label_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        logger.info("[{}] Dictionary: {} types".format(args.label_suffix, len(vocab) - 1))
        n_seq_tok = [0, 0]
        replaced = Counter()
    ...
    def make_binary_image_dataset(images_fnames, images_path, output_prefix, num_workers):
        ...
        pool = None
        if num_workers > 1:
            pool = ThreadPool(processes=num_workers - 1)  # Pool
            for worker_id in range(1, num_workers):
                prefix = "{}{}".format(output_prefix, worker_id)
                pool.apply_async(
                    binarize_images,
        ...
        merge_result(
            Binarizer.binarize_images(
                images_fnames, images_path, utils.parse_image, lambda t: ds.add_item(t),
                offset=0, end=offsets[1]
            )
        )
        if num_workers > 1:
    ...
    # <<fix>>
    def make_all_label(lang, vocab):
        if args.trainpref:
            make_binary_label_dataset(vocab, args.trainpref+"."+args.label_suffix, "train"+"."+args.label_suffix, lang, num_workers=args.workers)
        if args.validpref:
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(k) if k > 0 else "valid"
                make_binary_label_dataset(vocab, validpref+"."+args.label_suffix, outprefix+"."+args.label_suffix, lang, num_workers=args.workers)
        if args.testpref:
            for k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "test{}".format(k) if k > 0 else "test"
                make_binary_label_dataset(vocab, testpref+"."+args.label_suffix, outprefix+"."+args.label_suffix, lang, num_workers=args.workers)
    ...
    if args.label_suffix:
        make_all_label(None, tgt_dict)
    image = os.path.exists(args.images_path)
    if args.image_suffix and image:
        make_all_images(args.image_suffix)
...
def binarize_label(args, filename, vocab, output_prefix, lang, offset, end, append_eos=False):
    ds = indexed_dataset.make_builder(dataset_dest_file(args, output_prefix, lang, "bin"),
                                      impl=args.dataset_impl, vocab_size=len(vocab))
...
def binarize_images(args, filename, path, parse_image, output_prefix, offset, end):
    ds = indexed_dataset.make_builder(dataset_dest_file(args, output_prefix, None, "bin"),
                                      impl=args.dataset_impl, image=True, vocab_size=None)
