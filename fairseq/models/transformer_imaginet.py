...
@register_model("transformer_imaginet")  # <<fix>>
class TransformerImaginetModel(FairseqEncoderDecoderModel):  # <<fix>>
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # <fix>
        return TransformerImaginetEncoder(args, src_dict, embed_tokens)
    ...
    def forward(
        ...
        # <fix>, img_feats: ture image veteor v; x: encoder output hi -> src_len, bsz, dim
        img_feats = self.encoder.visual_features(image)  # B*img_dim
        x = encoder_out.encoder_out
        pred_img_feats = self.encoder.encoder_image(x.mean(dim=0))
        pred_img_norm = pred_img_feats / pred_img_feats.norm(2, dim=1, keepdim=True)
        img_norm = img_feats / img_feats.norm(2, dim=1, keepdim=True)
        image_similarity = torch.matmul(pred_img_norm, img_norm.T)
        ...
        return decoder_out, image_similarity  # <fix>
...
class ImageGlobalFeaturesProjector(nn.Module):
    ...
    def __init__(self, num_layers, nfeats, outdim, dropout,
        ...
        layers.append(nn.Linear(outdim * num_layers, nfeats)) # <fix>
        if use_nonlinear_projection:
            layers.append(nn.Tanh())
        # layers.append(nn.Dropout(dropout))
...
class TransformerImaginetEncoder(FairseqEncoder):  # <<fix>>
    """
    ...
    def forward(
        ...
        # <fix>, img_feats: ture image veteor v, x: encoder output hi -> src_len, bsz, dim -> x_mean: bsz, dim
        # pred_img_feats = self.encoder_image(x.mean(dim=0))
        # pred_img_norm = pred_img_feats / pred_img_feats.norm(2,  dim=1, keepdim=True)
        # img_norm = img_feats / img_feats.norm(2, dim=1, keepdim=True)
        # image_similarity = torch.matmul(pred_img_norm, img_norm.T)
...
@register_model_architecture("transformer_imaginet", "transformer_imaginet")  # <<fix>>
...
@register_model_architecture("transformer_imaginet", "transformer_imaginet_iwslt_de_en")  # <<fix>>
def transformer_imaginet_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_imaginet", "transformer_imaginet_wmt_en_de")  # <<fix>>
def transformer_imaginet_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('transformer_imaginet', 'transformer_imaginet_tiny')  # <<fix>>
def transformer_imaginet_tiny(args):
    ...
    args.use_nonlinear_projection = getattr(args, "use_nonlinear_projection", True)
    args.multimodal_model_type = getattr(args, "multimodal_model_type", 'ammt')
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_imaginet", "transformer_imaginet_vaswani_wmt_en_de_big")  # <<fix>>
def transformer_imaginet_vaswani_wmt_en_de_big(args):
...
@register_model_architecture("transformer_imaginet", "transformer_imaginet_vaswani_wmt_en_fr_big")  # <<fix>>
def transformer_imaginet_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_imaginet_vaswani_wmt_en_de_big(args)


@register_model_architecture("transformer_imaginet", "transformer_imaginet_wmt_en_de_big")  # <<fix>>
def transformer_imaginet_wmt_en_de_big(args):
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_imaginet_vaswani_wmt_en_de_big(args)


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_imaginet", "transformer_imaginet_wmt_en_de_big_t2t")  # <<fix>>
def transformer_imaginet_wmt_en_de_big_t2t(args):
    args.encoder_normalize_before = getattr(args, "encoder_normalize_before", True)
    args.decoder_normalize_before = getattr(args, "decoder_normalize_before", True)
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
