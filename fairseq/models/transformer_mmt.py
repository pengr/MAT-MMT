...
@register_model("transformer_mmt")  # <<fix>>
class TransformerMMTModel(FairseqEncoderDecoderModel):  # <<fix>>
    """
    ...
    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens):  # <fix>
        return TransformerMMTEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerMMTDecoder(
            args,
            tgt_dict,
            embed_tokens,
...
class TransformerMMTEncoder(FairseqEncoder):  # <<fix>>
    """
    ...
    def build_encoder_layer(self, args):  # <fix>
        return TransformerMMTEncoderLayer(args)
    ...
    def forward(
        ...
        # <fix>, concat pseudo text features and text features, T' x B x C
        x_txt = x
        x = torch.cat([x_txt, img_proj], dim=0)

        # compute padding mask # <fix>
        encoder_padding_mask = src_tokens.eq(self.padding_idx)
        src_len, bsz, _ = x_txt.size()
        tgt_len, bsz, _ = x.size()
        expand_encoder_padding_mask = encoder_padding_mask.unsqueeze(1).expand(bsz, tgt_len, src_len)
        img_padding_mask = torch.zeros(img_proj.shape[1], img_proj.shape[0]).type_as(encoder_padding_mask)  # batch, img_len
        encoder_padding_mask = torch.cat([encoder_padding_mask, img_padding_mask], dim=1)# batch, src_len+img_len
        ...
        for layer in self.layers:
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                x = layer(x, x_txt, expand_encoder_padding_mask)  # <fix>
...
class TransformerMMTDecoder(FairseqIncrementalDecoder):
    """
...
　　　
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        return TransformerMMTDecoderLayer(args, no_encoder_attn)
    ...
    def extract_features(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            # <fix>
            src_len, bsz, _ = encoder_state.size()
            tgt_len, bsz, _ = x.size()
            expand_encoder_padding_mask = encoder_out.encoder_padding_mask.unsqueeze(1).expand(bsz, tgt_len, src_len)
            ...
            if not self.training or (dropout_probability > self.decoder_layerdrop):
                x, layer_attn, _ = layer(
                    x,
                    encoder_state,
                    expand_encoder_padding_mask # <fix>
                    if encoder_out is not None
                    else None,
                    incremental_state,
...
@register_model_architecture("transformer_mmt", "transformer_mmt")  # <<fix>>
...
@register_model_architecture("transformer_mmt", "transformer_mmt_iwslt_de_en")  # <<fix>>
def transformer_mmt_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("transformer_mmt", "transformer_mmt_wmt_en_de")  # <<fix>>
def transformer_mmt_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('transformer_mmt', 'transformer_mmt_tiny')  # <<fix>>
...
@register_model_architecture("transformer_mmt", "transformer_mmt_vaswani_wmt_en_de_big")  # <<fix>>
def transformer_mmt_vaswani_wmt_en_de_big(args):
...
@register_model_architecture("transformer_mmt", "transformer_mmt_vaswani_wmt_en_fr_big")  # <<fix>>
def transformer_mmt_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_mmt_vaswani_wmt_en_de_big(args)


@register_model_architecture("transformer_mmt", "transformer_mmt_wmt_en_de_big")  # <<fix>>
def transformer_mmt_wmt_en_de_big(args):
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    transformer_mmt_vaswani_wmt_en_de_big(args)


# default parameters used in tensor2tensor implementation
@register_model_architecture("transformer_mmt", "transformer_mmt_wmt_en_de_big_t2t")  # <<fix>>
def transformer_mmt_wmt_en_de_big_t2t(args):
    args.encoder_normalize_before = getattr(args, "encoder_normalize_before", True)
    args.decoder_normalize_before = getattr(args, "decoder_normalize_before", True)
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
